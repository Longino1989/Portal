<?php
$documento=$_POST["docAcusex"];
$user=$_POST["usuarioAcusex"];
...
//echo $condicion."|".$estatus."|".$numDias[0]; return -1;
...
//echo $fechaRevision."|".$fechaPago."|".$nuevaFechaPago; return -1;

$conteo = count($_FILES["archivosAcuse"]["name"]);
for ($i = 0; $i < $conteo; $i++) {

    //$sef = trim($sef);

$sef = strtr($_FILES['archivosAcuse']['name'][$i], "áéíóúÁÉÍÓÚñÑç", "aeiouAEIOUnNc");
...
$sef=str_replace('Ó','O',$sef);
    ...
    $uploaddir = '../../../archivosPDFAcuse/'.$documento;
    $nombre=$documento.basename($sef);

    //$info = new SplFileInfo($nombre);
    //$dato = var_dump($info->getExtension());
...
if($num_rows>0){
	...
	$nombre=$documento."-".$num.basename($sef);
        ...
        $num_rows = sqlsrv_num_rows($re);
	$nombre=$documento."-".$num.basename($sef);
	$uploaddir = '../../../archivosPDFAcuse/'.$documento."-".$num;}
...
//$nombreArchivo = $_FILES["archivosAcuse"]["tmp_name"][$i];
if(move_uploaded_file($_FILES['archivosAcuse']['tmp_name'][$i], $file)){
    ...
    VALUES(".$documento.",'".$nombre."','".$file."','3',GETDATE(),".$user.",'acuse',convert(varchar,'".$fechaRevision."',120),convert(varchar,'".$nuevaFechaPago."',120),'".$nombreDia."','MIERCOLES')");
