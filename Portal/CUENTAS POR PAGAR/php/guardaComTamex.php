...
$comx=sqlsrv_query($sd,"select comentariosTamex from tprovIntelisis where MovID = ".$movID."");
$com=sqlsrv_fetch_array($comx);
if($com["comentariosTamex"]=='' || $com["comentariosTamex"]==' '){$cc="comentariosTamex=' - ".$user["usuario"].": '+Convert(varchar(17), GETDATE()+'".$horas."', 13)+ ' ".$comentario."'";
}else{$cc="comentariosTamex=comentariosTamex+'
...
$NuevoComx=sqlsrv_query($sd,"select comentariosTamex from tprovIntelisis where MovID = ".$movID."");
$Ncom=sqlsrv_fetch_array($NuevoComx);

echo "||".$Ncom["comentariosTamex"]."";
